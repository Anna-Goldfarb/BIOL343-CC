...
1. **Anna Goldfarb** — 20455917
    + [BIOL343_CC](https://github.com/Anna-Goldfarb/BIOL343_CC)
1. **Zaidie Daibel** -
    + [BIOL343_CC](https://github.com/your_name/BIOL343_CC)
1. **Ellie Maki** -
    + [BIOL343_CC](https://github.com/your_name/BIOL343_CC)
1. **Henry Moore** -
    + [BIOL343_CC](https://github.com/your_name/BIOL343_CC)
...
---------|-------------|------
population_type | sexual or asexual | S or A
ovary_width | ovary width of flower | mm
ovary_height | ovary height of flower | mm
flower_number | count | amount of flowers
...
```{r import-clean}
cc1_data <- read.csv("/Users/annagoldfarb/Desktop/BIOL343/BIOL343-CC/data/A1_Data_Excel2.csv")
cc1_data$ovary_area <- cc1_data$ovary_width_mm * cc1_data$ovary_height_mm
```
...
```{r area-means, include=FALSE}
area_means <- cc1_data %>% aggregate(ovary_area ~ population_type, mean)
...
kable(area_means, caption = "Mean Ovary Area of Population Types")
```
...
```{r area-histogram, echo=FALSE, message=FALSE}
MyPlot <- ggplot(aes(x=ovary_area), data=cc1_data) + geom_histogram(binwidth=0.7) + labs(title="Ovary Area by Population type") + xlab("Ovary Area (mm^2)") + ylab("Population Type (A/S)") + theme_classic()
MyPlot + facet_wrap(vars(population_type))
```
...
![Solanum pimpinellifolium](./images/images.jpg)

[Source: ](https://en.wikipedia.org/wiki/Solanum_pimpinellifolium)
